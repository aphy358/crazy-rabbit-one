		...
		<table class="hotel-price-table" v-if="combinedRows.length">
			...
			<transition name="slide-price-row" v-for="(priceRow, i) in combinedRows" :key="i" v-if="priceRow.isShow"
				...
				<tr class="hotel-price-tr">
					<td v-if="priceRow.rowSpan" class="first-td" :rowspan="priceRow.rowSpan">
						<div>{{priceRow.rowSpanText}}</div>
					</td>
					<td :class="priceRow.tdBindClass" @click="toggleSlideRow(priceRow)">
						<div class="hotel-price-room-name" :class="priceRow.roomNameBindClass">
							<el-popover placement="top-start" width="245" trigger="hover"
                      ...
                      {{priceRow.roomName}}
							...
							<span v-if="priceRow.rowsDropDown" class="room-type-icon-outer"
					...
					<td class="align-center">
						<div>
							<p v-if="priceRow.bedTypeName"><span
									:title="priceRow.bedTypeName.split('[')[0]">{{ priceRow.bedTypeName.split('[')[0]
								}}</span></p>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //生成链接
      getLink(priceRow, i) {
        let priceKey = `hotelPriceStrs_${+new Date()}${i}`;
        this.priceKeyArr[i] = priceKey;
        let ch = this.$store.state.cityType;
        let checkin = this.$store.state.checkin;
        let checkout = this.$store.state.checkout;
        let adultNum = this.$store.state.adultNum;
        let childrenNum = this.$store.state.childrenNum;
        let childrenStr = this.$store.state.childrenStr;
        let roomNum = this.$store.state.roomNum;
        
        let obj = {
          staticInfoId: priceRow.hotelId,
          hotelId: priceRow.hotelId,
          supplierId: priceRow.supplierId,
          roomId: priceRow.roomId,
          breakFastId: priceRow.breakFastId,
          paymentType: priceRow.paymentType,
          rateType: priceRow.rateType,
          isQueryPrice: true,
          startDate: checkin,
          endDate: checkout,
          citytype: ch,
          ...
          childrenAgeStr: childrenStr,
          roomNum: roomNum,
          ch: ch,
          hotelPriceStrsKey: priceKey,
        };
        
        if (priceRow.hasOwnProperty('isHasMarketing') && priceRow.isHasMarketing === 1) {
          obj.isHasMarketing = priceRow.isHasMarketing;
          obj.marketingPrice = priceRow.marketing.marketingPrice;
          obj.startTime = priceRow.marketing.startTime;
          obj.endTime = priceRow.marketing.endTime;
        }
      ...
      setHotelPriceStrs(priceRow, i) {
        let priceKey = this.priceKeyArr[i];
        sessionStorage.setItem(priceKey, encodeURIComponent(window.JSON.stringify(priceRow)));
      }
